...
[![IRC](https://img.shields.io/badge/OFTC_IRC-%23Zeon-blue)](https://webchat.oftc.net/?nick=&channels=%23Zeon)

![Zeon Boot Demo](etc/Zeon_demo.gif)

**Zeon** is a Unix-like, Linux-compatible kernel written in Rust and AArch64
assembly.

It features an asynchronous kernel core, a modular architecture abstraction
layer, and binary compatibility with Linux userspace applications. Zeon is
...
### Async Core
One of the defining features of `Zeon` is its usage of Rust's `async/await`
...
* Drivers:
    ...
    * NVMe block driver for high-performance SSDs
    * VirtIO block driver for paravirtualized storage

### Hardware Drivers
* **Display/Video**: VirtIO GPU, framebuffer support
* **Network**: VirtIO network driver (virtio-net)
* **Storage**: NVMe, VirtIO block devices
* **USB**: USB Host Controller (OHCI, EHCI, xHCI)
* **Audio**: HD Audio, VirtIO sound device
* **GPU**: GPU device interface with memory management
* **I2C**: I2C bus master driver
* **SPI**: SPI bus master driver
* **Serial**: PL011 UART, BCM2835 AUX
* **Timer**: ARMv8 architecture timers
* **RTC**: PL031 real-time clock
* **Interrupt**: ARM GIC v2/v3 interrupt controllers
* **RNG**: VirtIO random number generator

## `libkernel` & Testing
`Zeon` is built on top of `libkernel`, a utility library designed to be
...
The kernel runs off of `Zeon.img`.
...
Zeon is under active development. Current focus areas include:
...
Zeon is an experimental kernel focused on exploring asynchronous design and
...
| `make toolchain` | Install ARM64 toolchain |

### GN Build System
```bash
gn gen out/gn          # Generate build files
ninja -C out/gn        # Build all targets
ninja -C out/gn zeon  # Build kernel only
```
